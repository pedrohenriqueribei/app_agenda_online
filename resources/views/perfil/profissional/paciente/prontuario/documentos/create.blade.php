...
@section('content')
    ...
    <h1 class="titulo_1">Registro de Documetnos do Paciente</h1>
    ...
    <form action="{{ route('perfil.profissional.paciente.prontuario.psicologico.documentos.store', [$profissional, $paciente, $prontuario_psicologico]) }}" method="POST" class="space-y-6">
        @csrf
        ...
        <div>
            ...
            <textarea name="finalidade" id="finalidade" rows="3"
                class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm 
                    focus:ring-blue-500 focus:border-blue-500">{{ old('finalidade') }}</textarea>
        </div>
        ...
        <div>
            ...
            <input type="text" name="destinatario" id="destinatario"
                class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm 
                    focus:ring-blue-500 focus:border-blue-500" />{{ old('destinatario') }}
        </div>

        <!-- DATA DA EMISSÃO -->
        <div>
            <label for="data_emissao" class="block font-medium text-gray-700">Data da Emissão</label>
            <input type="date" name="data_emissao" id="data_emissao" class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm"/>
        </div>        
