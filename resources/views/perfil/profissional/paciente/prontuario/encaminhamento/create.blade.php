...
@section('title', 'Registro de Encaminhamento do Paciente')

@section('content')

    <h1 class="titulo_1">Registro de Encaminhamento do Paciente</h1>
    ...
    <form action="{{ route('perfil.profissional.paciente.prontuario.psicologico.encaminhamento.store', [$profissional, $paciente, $prontuario_psicologico]) }}" method="POST" class="space-y-6">
        @csrf
        ...
        <div>
            ...
            <textarea name="descricao" id="descricao" rows="3"
                class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm 
                    focus:ring-blue-500 focus:border-blue-500">{{ old('descricao') }}</textarea>
        </div>
        ...
        <div>
            ...
            <select name="tipo" id="tipo"
                ...
                <option value="encaminhamento" {{ old('tipo') == 'encaminhamento' ? 'selected' : '' }}>Encaminhamento</option>
                <option value="encerramento" {{ old('tipo') == 'encerramento' ? 'selected' : '' }}>Encerramento</option>
            </select>
        </div>        
