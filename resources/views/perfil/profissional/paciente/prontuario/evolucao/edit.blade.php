...
@section('title', 'Atualizar Registro de evolução do paciente')

@section('content')

    <h1 class="titulo_1">Atualizar Registro de Evolução do Trabalho</h1>
    ...
    <form action="{{ route('perfil.profissional.paciente.prontuario.psicologico.evolucao.update', [$profissional, $paciente, $prontuario_psicologico, $registro_evolucao]) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')

        <!-- DESCRIÇÃO -->
        <div>
            <label for="descricao" class="block font-medium text-gray-700">Descrição da Evolução</label>
            <textarea name="descricao" id="descricao" rows="3"
                class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm 
                    focus:ring-blue-500 focus:border-blue-500">{{ old('descricao', $registro_evolucao->descricao) }}</textarea>
        </div>
        ...
        <!-- ✅ Botão -->
        <div class="flex justify-center">
            <button type="submit" class="btn btn-warning">Atualizar Evolução</button>
        </div>
    </form>
