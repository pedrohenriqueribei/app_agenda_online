...
@section('title', 'Atualizar Registro de Instrumentos do Paciente')

@section('content')

    <h1 class="titulo_1">Atualizar Registro de Instrumentos de avaliação Psicológica</h1>
    ...
    <form action="{{ route('perfil.profissional.paciente.prontuario.psicologico.instrumentos.update', [$profissional, $paciente, $prontuario_psicologico, $registro_instrumento]) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <textarea name="instrumento_avaliacao_psi" id="instrumento_avaliacao_psi" rows="3"
                class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm 
                    focus:ring-blue-500 focus:border-blue-500">{{ old('instrumento_avaliacao_psi', $registro_instrumento->instrumento_avaliacao_psi) }}</textarea>
        </div>        
        ...
        <!-- ✅ Botão -->
        <div class="flex justify-center">
            <button type="submit" class="btn btn-warning">Atualizar Instrumento</button>
        </div>
    </form>
